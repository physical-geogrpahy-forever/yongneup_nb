...
# LAI/AGB threshold. Frozen NPP<=0 is maladaptation of the fixed 120-ka strategy,
# not a bare-state criterion.
anchor = 'const SNAPSHOTS=Set([120,100,80,60,40,20,10,5,0])\n'
insert = '''const H_OPT_LOWER_BOUND=1.0\nconst H_OPT_BOUND_TOL=1e-3\n\nfunction dynamic_optimizer_pathological(mode::String, ev)\n    mode=="dynamic" && (!isfinite(ev.npp) || ev.npp<=0.0) && isfinite(ev.tr.H) && ev.tr.H <= H_OPT_LOWER_BOUND + H_OPT_BOUND_TOL\nend\n\nfunction frozen_maladapted(mode::String, ev)\n    mode=="frozen" && (!isfinite(ev.npp) || ev.npp<=0.0)\nend\n'''
assert s.count(anchor) == 1
s = s.replace(anchor, anchor + insert)

# Apply realized AET=0 only to the audited dynamic optimizer pathology, inside
# the soil-water fixed point. Frozen maladaptation retains fixed traits and its
# physiological AET/NPP response exactly as in the production ablation.
old = '        rows,jan_new=simulate_bucket(cap,jan,precip,days,Float64.(ev.gpp.AET_monthly_mm)); es_new=[r.esupply_mm_day for r in rows]'
new = '''        optimizer_pathological=dynamic_optimizer_pathological(mode,ev)\n        aet_req=optimizer_pathological ? zeros(Float64,12) : Float64.(ev.gpp.AET_monthly_mm)\n        rows,jan_new=simulate_bucket(cap,jan,precip,days,aet_req); es_new=[r.esupply_mm_day for r in rows]'''
assert s.count(old) == 1
s = s.replace(old,new)

old = '            finalrows,jan_final=simulate_bucket(cap,jan,precip,days,Float64.(final.gpp.AET_monthly_mm))'
new = '''            optimizer_pathological_final=dynamic_optimizer_pathological(mode,final)\n            aet_req_final=optimizer_pathological_final ? zeros(Float64,12) : Float64.(final.gpp.AET_monthly_mm)\n            finalrows,jan_final=simulate_bucket(cap,jan,precip,days,aet_req_final)'''
assert s.count(old) == 1
s = s.replace(old,new)

# Replace realized summary block. Dynamic boundary failures are realized as zero
# woody vegetation but latent traits/raw NPP are retained for audit. Frozen
# maladaptation is flagged only; its standing pools and physiological fluxes are
# preserved so the frozen-trait experiment remains an ablation rather than a
# forced-bare counterfactual.
...
summary_replacement = '''        tr=ev.tr; new_es[cell]=Float64.(conv.es); new_jan[cell]=Float64(conv.jan)\n        optimizer_pathological=dynamic_optimizer_pathological(mode,ev)\n        maladapted_frozen=frozen_maladapted(mode,ev)\n        woody_productive=!optimizer_pathological && isfinite(ev.npp) && ev.npp>0.0 && isfinite(tr.H) && tr.H>0.0\n        H_out=optimizer_pathological ? 0.0 : tr.H\n        a_ll_out=optimizer_pathological ? 0.0 : tr.a_ll\n        C_leaf_out=optimizer_pathological ? 0.0 : tr.C_leaf\n        LAI_out=optimizer_pathological ? 0.0 : tr.LAI\n        FPC_out=optimizer_pathological ? 0.0 : tr.FPC\n        GPP_out=optimizer_pathological ? 0.0 : ev.gpp.GPP\n        NPP_out=optimizer_pathological ? 0.0 : ev.npp\n        AET_out=optimizer_pathological ? 0.0 : ev.gpp.AET\n        Net_out=optimizer_pathological ? 0.0 : ev.net\n        AGB_out=optimizer_pathological ? 0.0 : ev.agb\n        BGB_out=optimizer_pathological ? 0.0 : ev.bgb\n        push!(summary,(cell_id=cell,row=Int(df.row[1]),col=Int(df.col[1]),soil_depth_m=Float64(df.soil_depth_m[1]),awc_capacity_mm=conv.capacity,\n            converged=conv.converged,iterations=conv.iterations,convergence_error=conv.error,optimizer_pathological=optimizer_pathological,maladapted_frozen=maladapted_frozen,woody_productive=woody_productive,latent_H_m=tr.H,latent_a_ll_yr=tr.a_ll,latent_C_leaf_gC_individual=tr.C_leaf,raw_NPP_gC_m2_yr=ev.npp,\n            H_m=H_out,a_ll_yr=a_ll_out,C_leaf_gC_individual=C_leaf_out,seasonality=tr.seasonality,r_s_r=tr.r_s_r,LAI=LAI_out,FPC=FPC_out,GPP_gC_m2_yr=GPP_out,NPP_gC_m2_yr=NPP_out,AET_mm_yr=AET_out,\n            Net_C_gain_gC_m2_yr=Net_out,AGB_C_g_m2_component_sum=AGB_out,BGB_C_g_m2_component_sum=BGB_out))'''
...
("npp=np.full(shape,np.nan,float); aet=np.full((12,)+shape,np.nan,float); lai=np.full(shape,np.nan,float); htrait=np.full(shape,np.nan,float); all_yr=np.full(shape,np.nan,float); cleaf=np.full(shape,np.nan,float)",
 "npp=np.full(shape,np.nan,float); aet=np.full((12,)+shape,np.nan,float); lai=np.full(shape,np.nan,float); htrait=np.full(shape,np.nan,float); all_yr=np.full(shape,np.nan,float); cleaf=np.full(shape,np.nan,float); agb_c=np.full(shape,np.nan,float); raw_npp=np.full(shape,np.nan,float); woody_productive=np.full(shape,False,bool); optimizer_pathological=np.full(shape,False,bool); maladapted_frozen=np.full(shape,False,bool)"),
...
("    return npp,aet,lai,htrait,all_yr,cleaf",
 "    return npp,aet,lai,htrait,all_yr,cleaf,agb_c,raw_npp,woody_productive,optimizer_pathological,maladapted_frozen"),
("    npp,aet,lai,htrait,all_yr,cleaf=assemble(summary,monthly,z.shape)",
 "    npp,aet,lai,htrait,all_yr,cleaf,agb_c,raw_npp,woody_productive,optimizer_pathological,maladapted_frozen=assemble(summary,monthly,z.shape)"),
...
("'mean_eemt_MJ_m2_yr':float(np.nanmean(eemt[land])),'mean_LAI':",
 "'mean_eemt_MJ_m2_yr':float(np.nanmean(eemt[land])),'mean_agb_C_g_m2':float(np.nanmean(agb_c[land])),'mean_agb_dry_kg_m2':float(np.nanmean(agb[land])),'optimizer_pathological_cells':int(np.count_nonzero(land & optimizer_pathological)),'maladapted_frozen_cells':int(np.count_nonzero(land & maladapted_frozen)),'unproductive_woody_cells':int(np.count_nonzero(land & (h>0) & np.isfinite(raw_npp) & (~woody_productive))),'mean_LAI':"),
("'EEMT_MJ_m2_yr':eemt[rr,cc],'LAI':lai[rr,cc]",
 "'EEMT_MJ_m2_yr':eemt[rr,cc],'raw_NPP_gC_m2_yr':raw_npp[rr,cc],'woody_productive':bool(woody_productive[rr,cc]),'optimizer_pathological':bool(optimizer_pathological[rr,cc]),'maladapted_frozen':bool(maladapted_frozen[rr,cc]),'AGB_C_g_m2_component_sum':agb_c[rr,cc],'Pelletier_AGB_dry_kg_m2':agb[rr,cc],'LAI':lai[rr,cc]")
]
